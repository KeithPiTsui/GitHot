...
open class ValueCellDataSource: NSObject, UICollectionViewDataSource, UITableViewDataSource {
  ...
  public var valueSnapshot: [[Any]] {return self.values.map {$0.map(first)}}
  /**
  ...
  public final func collectionViewTry(_ collectionView: UICollectionView,
                                   cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {

    let (value, _) = self.values[indexPath.section][indexPath.item]

//    let cell = collectionView.dequeueReusableCell(withReuseIdentifier: reusableId, for: indexPath)
    let cell = RepoCollectionViewCell(frame: .zero)
