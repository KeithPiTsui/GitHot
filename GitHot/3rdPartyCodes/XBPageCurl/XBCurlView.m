...
- (BOOL)initialize
{
    ...
    [self createVertexBufferWithXRes:(GLuint)self.horizontalResolution
                                yRes:(GLuint)self.verticalResolution];
...
- (void)setCylinderRadius:(CGFloat)cylinderRadius animatedWithDuration:(NSTimeInterval)duration interpolator:(double (^)(double t))interpolator completion:(void (^)(void))completion
{
    ...
    __weak XBCurlView *weakSelf = self;
    XBAnimation *animation = [XBAnimation animationWithName:kCylinderRadiusAnimationName
                                                   duration:duration
                                                     update:^(double t) {
